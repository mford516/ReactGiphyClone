...
class Create extends Component {
    ...
    createGif = () => {
        let cloudinarySettings = {
            cloud_name: 'dhsmhnbdz',
            upload_preset: 'we1lrixd',
            sources: ['local'],
    ...
    setGifString = (uploadedVideoId) => {
        this.setState({
            gifUrl: `http://res.cloudinary.com/dhsmhnbdz/video/upload${(this.state.startTime > 0 && this.state.endTime > 0) ? '/so_' + this.state.startTime + ',eo_' + this.state.endTime : ''}/${uploadedVideoId}.gif`
        });
    }
    render() {
        return (
